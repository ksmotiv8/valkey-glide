...
class TestScopeDisconnectionBehavior:
    ...
    @pytest.mark.parametrize("cluster_mode", [True, False])
    def test_scope_fails_after_connection_killed(self, cluster_mode):
    ...
    @pytest.mark.parametrize("cluster_mode", [True, False])
    def test_broken_scope_does_not_pollute_pool(self, cluster_mode):
    ...
    @pytest.mark.parametrize("cluster_mode", [True, False])
    def test_scope_no_auto_reconnect(self, cluster_mode):
        """Scoped connections do not transparently reconnect — they fail fast."""
        client = _create_client(cluster_mode, request_timeout=2000)
