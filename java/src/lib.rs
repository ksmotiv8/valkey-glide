...
pub extern "system" fn Java_glide_internal_GlideNativeBridge_executeBatchDirect(
    ...
    run_ffi(|| {
        ...
        let cmd_count = match env.get_array_length(&request_types) {
            ...
            Err(e) => {
                jni_client::complete_error_sync(
                    &mut env, callback_id, &format!("Failed to get request types length: {e}"), 0,
                );
                return Some(());
            }
        };
        ...
        if let Err(e) = env.get_int_array_region(&request_types, 0, &mut req_types) {
            jni_client::complete_error_sync(
                &mut env, callback_id, &format!("Failed to read request types: {e}"), 0,
            );
            return Some(());
        }
        ...
        let all_args = match all_args {
            ...
            Err(e) => {
                jni_client::complete_error_sync(
                    &mut env, callback_id, &format!("Failed to extract batch args: {e}"), 0,
                );
                return Some(());
            }
        };
        ...
        let timeout_val = if timeout > 0 { Some(timeout as u32) } else { None };
                ...
                Ok(mut client) => {
                    let result: Result<redis::Value, redis::RedisError> = async {
                        ...
                        for (i, rt) in req_types.iter().enumerate() {
                            ...
                            >::from_i32(*rt);
                            let request_type: glide_core::request_type::RequestType = proto_rt.into();
                        ...
                        let exec_res = if is_atomic_bool {
                            client
                                .send_transaction(&pipeline, routing, timeout_val, raise_on_error_bool)
                                .await
                        } else {
                            client
                                .send_pipeline(
                ...
                Err(err) => {
                    complete_callback(
                        ...
                        Err(redis::RedisError::from((
                            redis::ErrorKind::ClientError, "Client not found", err.to_string(),
                        ))),
                        !expect_utf8_bool,
                    );
                }
